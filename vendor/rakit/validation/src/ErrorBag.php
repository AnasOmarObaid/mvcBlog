...
class ErrorBag
{
     ...
     * @return void
     */
    static public function add(string $key, string $rule, string $message)
    {
        if (!isset(Self::$messages[$key])) {
            Self::$messages[$key] = [];
        }

        Self::$messages[$key][$rule] = $message;
    }
    ...
    public function has(string $key): bool
    {
        ...
        if ($this->isWildcardKey($key)) {
            ...
            $messages = isset(Self::$messages[$key])? Self::$messages[$key] : null;
    ...
    public function first(string $key)
    {
        ...
        if ($this->isWildcardKey($key)) {
            ...
            $keyMessages = isset(Self::$messages[$key])? Self::$messages[$key] : [];
     ...
     * @return array
     */
    static public function firstOfAll(string $format = ':message', bool $dotNotation = false): array
    {
        $messages = Self::$messages;
        $results = [];
        foreach ($messages as $key => $keyMessages) {
            if ($dotNotation) {
                $results[$key] = Self::formatMessage(array_shift($messages[$key]), $format);
            } else {
                Helper::arraySet($results, $key, Self::formatMessage(array_shift($messages[$key]), $format));
            }
        }
        return $results;
    }
     ...
     * @return string
     */
    static protected function formatMessage(string $message, string $format): string
    {
